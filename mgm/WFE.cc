                            ...
                            const std::string& prepareActivity, std::string& errorMsg)
{
  ...
  eosLog.addParam(eosLog.LOG, std::string(gOFS->logId));
  eosLog.addParam(eosLog.PATH, fullPath);
  eosLog.addParam(eosLog.RUID, mVid.uid);
  eosLog.addParam(eosLog.RGID, mVid.gid);
  eosLog.addParam(eosLog.TD, mVid.tident.c_str());
  eosLog.addParam(eosLog.PREP_WFE_EVENT, "stage");
  eosLog.addParam(eosLog.PREP_WFE_ACTIVITY, prepareActivity);
  ...
  if (gOFS->_stat(fullPath.c_str(), &buf, errInfo, mVid, nullptr, nullptr,
    ...
    eosLog.addParam(eosLog.PREP_WFE_ONDISK, onDisk);
    eosLog.addParam(eosLog.PREP_WFE_ONTAPE, onTape);
  } else {
    ...
    eosLog.addParam(eosLog.PREP_WFE_SENTTOCTA, false);
    eosLog.addParam(eosLog.PREP_WFE_ERROR, err_message.str());
  ...
  if (onDisk) {
    ...
    try {
      ...
      gOFS->eosView->updateFileStore(fmd.get());
      eosLog.addParam(eosLog.PREP_WFE_EVICTCOUNTER, evictionCounter);
    ...
    eosLog.addParam(eosLog.PREP_WFE_FIRSTPREPARE, isFirstPrepare);
    eosLog.addParam(eosLog.PREP_WFE_REQID, prepareRequestId);
    eosLog.addParam(eosLog.PREP_WFE_REQCOUNT, prepareReqIds.values.size());
    eosLog.addParam(eosLog.PREP_WFE_REQLIST, prepareReqIds.serialize());
  ...
  eosLog.addParam(eosLog.HOST, mgmHostName);
  ...
  eosLog.addParam(eosLog.PREP_WFE_SENTTOCTA, true);
  return sendResult;
}
    ...
    std::string& errorMsg)
{
  ...
  eosLog.addParam(eosLog.RGID, mVid.gid);
  eosLog.addParam(eosLog.TD, mVid.tident.c_str());
  eosLog.addParam(eosLog.PREP_WFE_EVENT, "abort");
  ...
  XattrSet prepareReqIds;
  {
    ...
    try {
      ...
      eosLog.addParam(eosLog.PREP_WFE_REQID, opaqueRequestId);
    ...
    std::string& errorMsg)
{
  ...
  eosLog.addParam(eosLog.RGID, mVid.gid);
  eosLog.addParam(eosLog.TD, mVid.tident.c_str());
  eosLog.addParam(eosLog.PREP_WFE_EVENT, "evict");
  ...
  if (gOFS->_stat(fullPath.c_str(), &buf, errInfo, mVid, nullptr, nullptr,
    ...
    eosLog.addParam(eosLog.PREP_WFE_SENTTOCTA, false);
    eosLog.addParam(eosLog.PREP_WFE_ERROR, msg.str());
    MoveWithResults(EAGAIN);
    return EAGAIN;
  }
