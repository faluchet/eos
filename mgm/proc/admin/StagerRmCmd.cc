  ...
  for (auto i = 0; i < stagerRm.file_size(); i++) {
    ...
    case eos::console::StagerRmProto::FileProto::kPath:
      ...
      if (0 == path.length()) {
        ...
        ret_c = EINVAL;
        eosLog.addParam(eosLog.STAGERRM_ERROR, errStream.str());
        continue;
      }
      eosLog.addParam(eosLog.PATH, path);
      break;
    ...
    try {
      ...
      eosLog.addParam(eosLog.STAGERRM_EVICTCOUNTER, evictionCounter);
    ...
    if (evictionCounter > 0) {
      eosLog.addParam(eosLog.STAGERRM_FILEREMOVED, false);
      continue;
    }
    ...
    if (gOFS->_dropallstripes(path.c_str(), errInfo, root_vid, false) != 0) {
      ...
      eosLog.addParam(eosLog.STAGERRM_FILEREMOVED, true);
    }
  }
